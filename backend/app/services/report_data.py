...
def _income_rows(
    ...
    # Sum paid amounts per tenant per (year, month)
    paid_rows = (
        db.query(
    ...
    # Tenant id → apartment
    tenant_to_apt: dict[UUID, Apartment] = {t.id: apartments[i] for i, (apt_id, t) in
                                             enumerate(((apt_id, tenants_by_apt[apt_id])
                                                        for apt_id in [a.id for a in apartments]
                                                        if apt_id in tenants_by_apt))}
