    ...
    <div className="bg-white border border-slate-200 rounded-lg overflow-hidden" dir="rtl">
            ...
            <tr className="border-b border-slate-200">
              ...
              {monthList.map((m) => {
                ...
                return (
                  <th
                    key={`${m.year}-${m.month}`}
                    className={`px-2 py-3 text-center font-semibold ${isCurrent ? 'bg-blue-50 text-blue-700' : ''}`}
                  >
                ...
                return (
                  <tr key={ten.tenant_id} className="hover:bg-slate-50">
                    ...
                    {monthList.map((m) => {
                      ...
                      let cellClass = 'mx-auto w-12 h-12 rounded-lg flex flex-col items-center justify-center transition focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none';
                      let icon: string = '';
                      let iconClass = '';
                      let amountText = '';
                      ...
                      if (cellStatus === 'empty') {
                        ...
                        cellClass += ' bg-emerald-50 border border-emerald-200 cursor-pointer hover:scale-105';
                        icon = '✓';
                        iconClass = 'text-emerald-700 leading-none';
                        amountText = fmt(paid);
                        amountClass += ' text-slate-600';
                        title = t('collection.cell.editPayment');
                      } else if (cellStatus === 'partial') {
                        cellClass += ' bg-amber-50 border border-amber-200 cursor-pointer hover:scale-105';
                        icon = '◐';
                        iconClass = 'text-amber-700 leading-none';
                        amountText = fmt(paid);
                      ...
                      if (isHighlighted) cellClass += ' ring-2 ring-blue-500';
      ...
      <div className="border-t border-slate-200 px-4 py-3 bg-slate-50/50">
        ...
        <div className="flex flex-wrap gap-x-5 gap-y-2 text-[11px] text-slate-600">
          <LegendDot bg="bg-emerald-50 border border-emerald-200" iconClass="text-emerald-700" icon="✓" label={t('collection.matrixLegend.paid')} />
