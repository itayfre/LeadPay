...
export default function BuildingCardV2({ building, summary, trend, onClick, onEdit, onDelete }: Props) {
  ...
  return (
    <article
      className="group bg-white rounded-xl ring-1 ring-ink-200 shadow-sm hover:shadow-md transition p-5 flex flex-col gap-4 relative cursor-pointer"
      onClick={onClick}
    >
      <button
      ...
      <div className="grid grid-cols-3 gap-3 text-right">
        ...
        <div>
          ...
          <div className={`text-[15px] font-semibold tabular-nums mt-1 ${collected > 0 ? 'text-accent-700' : 'text-ink-400'}`}>
        ...
        <div>
          ...
          <div className={`text-[15px] font-semibold tabular-nums mt-1 ${overdue > 0 ? 'text-danger-600' : 'text-ink-400'}`}>
      ...
      <footer className="flex items-center justify-between pt-3 border-t border-ink-100">
        ...
        {trend && trend.length > 0 ? (
          ...
          <span className="text-[11px] text-ink-400">{t('buildings.card.noTrend')}</span>
        )}
      </footer>
